...
class Videollama3MetaForCausalLM(ABC):
    ...
    def average_pooling(self, mm_features, num_output_tokens):
        pool_size = mm_features.shape[0] // num_output_tokens 
        mm_features = mm_features[:pool_size * num_output_tokens]  # Trim excess if necessary
        pooled_features = mm_features.view(num_output_tokens, pool_size, -1).mean(dim=1)
        return pooled_features 
    
    def linear_projection_layer(self, mm_features, num_output_tokens):
        input_dim = mm_features.shape[0]
        output_dim = num_output_tokens
        linear_projection = nn.Linear(input_dim, output_dim, dtype=torch.bfloat16).to('cuda')
        projected_features = linear_projection(mm_features.transpose(0, 1)).transpose(0, 1) # Flip the mm_features according to the input
        assert projected_features.shape == (num_output_tokens, mm_features.shape[-1])
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        image_selected = (input_ids == self.config.image_token_index)
        if not self.eval:
            mm_features = self.linear_projection_layer(mm_features=mm_features, num_output_tokens=inputs_embeds[image_selected].shape[0])
        # mm_features = self.average_pooling(mm_features=mm_features, num_output_tokens=inputs_embeds[image_selected].shape[0]) # dynamic video wise compression
...
class Videollama3Qwen2ForCausalLM(Qwen2ForCausalLM, Videollama3MetaForCausalLM):
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        """
            self.eval values.
            True: When doing training or inference using the llava hound script
            False: When doing inference using an external script like in temp.py
        """

        self.eval = eval

        if inputs_embeds is None and not self.eval:
            (
                input_ids,
                attention_mask,
                position_ids,
                ...
                labels=labels,
                pixel_values=torch.cat(pixel_values[0], dim = 0), # encoded tensor based pixel values
                grid_sizes=grid_sizes,
                merge_sizes=merge_sizes,
                modals=pixel_values[1], # a list of 'video'
            )
        
        if inputs_embeds is None and self.eval:
            (
                input_ids,
                attention_mask,
                position_ids,
                ...
                past_key_values=past_key_values,
                labels=labels,
                pixel_values=pixel_values, # encoded tensor based pixel values
                grid_sizes=grid_sizes,
                merge_sizes=merge_sizes,
                modals=modals, # a list of 'video'
            )
